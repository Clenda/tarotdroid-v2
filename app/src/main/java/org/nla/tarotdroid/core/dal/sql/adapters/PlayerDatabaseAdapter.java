...
public class PlayerDatabaseAdapter extends BaseAdapter {
	...
	private void loadGamesPlayerListsFromRepository() {
		...
		while(dataSource.moveToNext()) {
			...
			Player player = this.playersByIdInRepository.get(dataSource.getLong(dataSource.getColumnIndex(
					DatabaseV5Constants.COL_GAME_PLAYERS_PLAYER_ID)));
	...
	private void loadGameSetsPlayerListsFromRepository() {
		...
		while(dataSource.moveToNext()) {
			...
			Player player = this.playersByIdInRepository.get(dataSource.getLong(dataSource.getColumnIndex(
					DatabaseV5Constants.COL_GAMESET_PLAYERS_PLAYER_ID)));
	...
	private Player fetchFromCursor(final Cursor dataSource) {
		...
		player.setCreationTs(new Date(dataSource.getLong(dataSource.getColumnIndex(
				DatabaseV5Constants.COL_PLAYER_TIMESTAMP))));
	...
	/* (non-Javadoc)
	 * @see org.nla.tarotdroid.core.dal.sql.adapters.BaseAdapter#clear()
	 */
	@Override
	public void clear() {
