...
//		values.put(DatabaseV5Constants.COL_TRACKER_ENTITY_CLOUD_ID, toTrack.getCloudId());
	...
	public Map<ObjectTypes, Map<ActionTypes, List<String>>> getTrackedEntities() {
		...
		while(dataSource.moveToNext()) {
			ActionTypes actionType = ActionTypes.valueOf(dataSource.getString(dataSource.getColumnIndex(
					DatabaseV5Constants.COL_TRACKER_ACTION)));
			ObjectTypes objectType = ObjectTypes.valueOf(dataSource.getString(dataSource.getColumnIndex(
					DatabaseV5Constants.COL_TRACKER_ENTITY_TYPE)));
