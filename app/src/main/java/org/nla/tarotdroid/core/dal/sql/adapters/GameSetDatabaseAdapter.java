...
public class GameSetDatabaseAdapter extends BaseAdapter {
	...
	private GameSet createFromCursor(Cursor dataSource) {
		...
		gameSet.setCreationTs(new Date(dataSource.getLong(dataSource.getColumnIndex(
				DatabaseV5Constants.COL_GAMESET_TIMESTAMP))));
		gameSet.setUuid(dataSource.getString(dataSource.getColumnIndex(DatabaseV5Constants.COL_GAMESET_UUID)));
		gameSet.setGameStyleType((GameStyleType.valueOf(dataSource.getString(dataSource.getColumnIndex(
				DatabaseV5Constants.COL_GAMESET_TYPE)))));
