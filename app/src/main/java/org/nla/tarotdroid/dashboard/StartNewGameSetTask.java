...
public class StartNewGameSetTask extends AsyncTask<Void, Void, Void> {
    ...
    public StartNewGameSetTask(
            final Activity activity, final ProgressDialog dialog, final GameSet gameSet,
    ...
    protected final Void doInBackground(final Void... voids) {
        ...
        try {
            // TODO Use context
            gameSetToBeStored = gameSetToBeStored || (!BuildConfig.IS_FULL && dalService.getGameSetCount() < 5);
            if (gameSetToBeStored) {
                // TODO Use context
                dalService.saveGameSet(this.gameSet);
            }
        } catch (Exception e) {
