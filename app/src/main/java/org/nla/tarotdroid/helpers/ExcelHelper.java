...
public final class ExcelHelper {
    ...
    public static String getPath(final GameSet gameSet) {
        return Environment.getExternalStorageDirectory()
                          .getAbsolutePath() + "/" + ExcelHelper.FORMATTER.format(gameSet.getCreationTs()) + ".xls";
    }
    ...
    public static String exportToExcel(
        ...
        File excelFile = new File(tarotDroidDir,
    ...
    public static String exportToExcel(
        ...
        // generate reports
        WritableSheet globalSheet = workbook.createSheet(ExcelHelper.FORMATTER.format(gameSet.getCreationTs()) + "Global",
                                                         startIndex);
        WritableSheet deltaSheet = workbook.createSheet(ExcelHelper.FORMATTER.format(gameSet.getCreationTs()) + "Delta",
                                                        startIndex + 1);
        ...
        globalSheet.addCell(new Label(gameSet.getPlayers().size() + 1,
                                      0,
                                      ExcelHelper.strLeaderPlayer));
        deltaSheet.addCell(new Label(gameSet.getPlayers().size() + 1,
                                     0,
                                     ExcelHelper.strLeaderPlayer));
        if (gameSet.getGameStyleType() == GameStyleType.Tarot5) {
            globalSheet.addCell(new Label(gameSet.getPlayers().size() + 2,
                                          0,
                                          ExcelHelper.strCalledPlayer));
            deltaSheet.addCell(new Label(gameSet.getPlayers().size() + 2,
                                         0,
                                         ExcelHelper.strCalledPlayer));
            globalSheet.addCell(new Label(gameSet.getPlayers().size() + 3,
                                          0,
                                          ExcelHelper.strCalledColor));
        ...
        for (BaseGame game : gameSet.getGames()) {
            if (game instanceof StandardTarot5Game) {
                ExcelHelper.exportStandardTarot5Game(context,
                                                     gameSet,
    ...
    private static void exportStandardGame(
            final Context context,
            final GameSet gameSet,
            final StandardBaseGame stdGame,
        ...
        for (int column = 1; column <= gameSet.getPlayers().size(); ++column) {
            ...
            if (!stdGame.getPlayers().contains(player)) {
                ...
                Number globalIndividualScore = new Number(column,
                                                          row,
                                                          gameSet.getGameSetScores()
                                                                 .getIndividualResultsAtGameOfIndex(
                                                                         stdGame.getIndex(),
                                                                         player));
                Number deltaIndividualScore = new Number(column,
                                                         row,
                                                         stdGame.getGameScores()
        ...
        globalSheet.addCell(new Label(gameSet.getPlayers().size() + 1,
                                      row,
                                      stdGame.getLeadingPlayer().getName()));
        deltaSheet.addCell(new Label(gameSet.getPlayers().size() + 1,
                                     row,
                                     stdGame.getLeadingPlayer().getName()));
    }

    private static void exportStandardTarot5Game(
            final Context context,
            final GameSet gameSet,
            final StandardTarot5Game std5Game,
            final WritableSheet globalSheet,
            final WritableSheet deltaSheet,
            final int row,
            final UIHelper uiHelper
        ...
        globalSheet.addCell(new Label(gameSet.getPlayers().size() + 1,
                                      row,
                                      std5Game.getLeadingPlayer().getName()));
        deltaSheet.addCell(new Label(gameSet.getPlayers().size() + 1,
                                     row,
                                     std5Game.getLeadingPlayer().getName()));
        ...
        globalSheet.addCell(new Label(gameSet.getPlayers().size() + 2,
                                      row,
                                      std5Game.getCalledPlayer().getName()));
        deltaSheet.addCell(new Label(gameSet.getPlayers().size() + 2,
                                     row,
                                     std5Game.getCalledPlayer().getName()));
    ...
    private static void exportBelgianGame(
            final Context context,
            final GameSet gameSet,
            final BaseGame game,
            final WritableSheet globalSheet,
        ...
        globalSheet.addCell(new Label(0,
                                      row,
                                      ExcelHelper.buildBelgianDescription(context,
                                                                          game.getIndex())));
        deltaSheet.addCell(new Label(0,
                                     row,
                                     ExcelHelper.buildBelgianDescription(context,
    ...
    private static String buildStandardDescription(
            final Context context,
            final int gameIndex,
            final BetType bet,
            final int points,
    ...
    private static void initializeStaticStrings(final Context context) {
        ExcelHelper.strLeaderPlayer = context.getResources()
                                             .getString(R.string.lblExportLeaderPlayer);
        ExcelHelper.strCalledColor = context.getResources()
                                            .getString(R.string.lblExportCalledColor);
        ExcelHelper.strCalledPlayer = context.getResources()
                                             .getString(R.string.lblExportCalledPlayer);
